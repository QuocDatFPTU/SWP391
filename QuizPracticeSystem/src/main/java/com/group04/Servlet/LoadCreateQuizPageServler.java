 ...
 * @author Admin
 */
@WebServlet(name = "LoadCreateQuizPageServler", urlPatterns = {"/LoadCreateQuizPageServler"})
public class LoadCreateQuizPageServler extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            //Create payload QuizDetail
            HttpSession session = request.getSession();
            QuizDetailDTO currentQuiz = new QuizDetailDTO();
            currentQuiz.setIsFillFull(false);
            session.setAttribute("QUIZ_DETAIL_ADMIN", currentQuiz);
            //Get subject list
            SubjectRepositoryImp dao = new SubjectRepositoryImp();
            List<Subject> listSubject = dao.getAllSubject();
            //Put List Subject into page
